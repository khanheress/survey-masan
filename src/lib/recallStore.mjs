...
export function initializeRecallStore(db) {
  db.exec(`CREATE TABLE IF NOT EXISTS recall_forms (
...
export function getRecallForm(db, id) {
  const form = db.prepare('SELECT * FROM recall_forms WHERE id = ?').get(id);
  if (!form) throw new RecallError('Không tìm thấy Form Recall.', 404);
  const slots = db.prepare(`SELECT s.starts_at, COUNT(b.id) AS booked_count FROM recall_slots s
...
export function saveRecallForm(db, input, user, id = null) {
  const settings = validateForm(input);
  return db.transaction(() => {
    if (id) {
      const existing = getRecallForm(db, id);
      ...
      db.prepare('UPDATE recall_forms SET title = ?, description = ?, allow_overlap = ?, is_open = ? WHERE id = ?')
        .run(settings.title, settings.description, settings.allow_overlap, settings.is_open, id);
      db.prepare('DELETE FROM recall_slots WHERE form_id = ?').run(id);
    } else {
      id = randomUUID();
      db.prepare('INSERT INTO recall_forms (id, title, description, share_token, allow_overlap, is_open, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)')
    ...
    for (const slot of settings.slots) insert.run(id, slot);
    return getRecallForm(db, id);
  }).immediate();
}

export function publicRecallForm(db, token, now = new Date()) {
  const found = db.prepare('SELECT id FROM recall_forms WHERE share_token = ?').get(token);
  if (!found) throw new RecallError('Link Form Recall không tồn tại.', 404);
  const form = getRecallForm(db, found.id);
...
export function bookRecall(db, token, input, now = new Date()) {
  ...
  return db.transaction(() => {
    const form = db.prepare('SELECT * FROM recall_forms WHERE share_token = ?').get(token);
    if (!form) throw new RecallError('Link Form Recall không tồn tại.', 404);
    if (!form.is_open) throw new RecallError('Form Recall hiện đã đóng đăng ký.', 410);
    const slot = db.prepare('SELECT starts_at FROM recall_slots WHERE form_id = ? AND starts_at = ?').get(form.id, input.starts_at);
    ...
    if (db.prepare('SELECT 1 FROM recall_bookings WHERE form_id = ? AND starts_at = ? AND phone = ?').get(form.id, slot.starts_at, phone)) {
      throw new RecallError('Số điện thoại này đã đăng ký khung giờ đã chọn.', 409);
    }
    if (!form.allow_overlap && db.prepare('SELECT 1 FROM recall_bookings WHERE form_id = ? AND starts_at = ?').get(form.id, slot.starts_at)) {
      throw new RecallError('Khung giờ vừa có người đăng ký. Vui lòng chọn giờ khác.', 409);
    }
    db.prepare('INSERT INTO recall_bookings (id, form_id, starts_at, name, phone) VALUES (?, ?, ?, ?, ?)')
