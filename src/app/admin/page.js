...
export default function DashboardPage(){
 const [currentYear]=useState(()=>Number(new Date(Date.now()+7*3600000).toISOString().slice(0,4)));
 ...
 return <div className="animate-fadeIn"><div className="page-header"><div><h1 className="page-title">Tổng quan</h1><p className="data-description">Số lượng phản hồi hợp lệ và tiến độ thu thập khảo sát.</p></div><button className="btn btn-secondary" onClick={refresh}>Làm mới</button></div>
  <section className="card" style={{marginBottom:'1.5rem'}}>
   <div className="flex-between" style={{gap:'1rem',flexWrap:'wrap'}}><h2 style={{fontSize:'1.25rem'}}>Phản hồi theo người mời trong tháng</h2>
    <div className="flex gap-2" style={{flexWrap:'wrap'}}>
     <label className="form-label" htmlFor="stats-month">Tháng<select id="stats-month" aria-label="Tháng thống kê" className="form-select" value={month.slice(5)} onChange={e=>setMonth(previous=>`${previous.slice(0,4)}-${e.target.value}`)}>{Array.from({length:12},(_,i)=><option key={i+1} value={String(i+1).padStart(2,'0')}>Tháng {i+1}</option>)}</select></label>
     <label className="form-label" htmlFor="stats-year">Năm<select id="stats-year" aria-label="Năm thống kê" className="form-select" value={month.slice(0,4)} onChange={e=>setMonth(previous=>`${e.target.value}-${previous.slice(5)}`)}>{Array.from({length:currentYear+11-1900},(_,i)=>currentYear+10-i).map(year=><option key={year} value={year}>{year}</option>)}</select></label>
    </div>
   </div>
   <p className="survey-flow-note">Tính theo số phản hồi đã lưu, theo giờ Việt Nam; một người gửi nhiều khảo sát được tính nhiều phản hồi.</p>
   {loading?<p role="status">Đang tải thống kê tháng {Number(month.slice(5))}/{month.slice(0,4)}…</p>:error?<p role="status">Chưa tải được thống kê tháng này. Hãy chọn lại tháng, năm hoặc bấm Thử lại.</p>:<div className="grid-cols-3">{data.monthly_inviters.map(item=><div className="card" key={item.inviter}><h3>{item.inviter}</h3><strong style={{fontSize:'1.8rem'}}>{item.count}</strong><span> phản hồi</span></div>)}</div>}
  </section>
