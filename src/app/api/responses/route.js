...
export async function GET(request) {
  try {
    ...
    const responses = db.prepare(query).all(...params).map(row => {
      let answers = {};
...
export async function POST(request) {
  try {
    ...
    if (respondent_phone) {
      const existing = db.prepare('SELECT * FROM responses WHERE survey_id = ? AND respondent_phone = ?').get(survey_id, respondent_phone);
      if (existing) {
    ...
    db.transaction(() => {
      insert.run(id, survey_id, project.id, respondent_phone || null, respondent_name || null, respondent_email || null, respondent_birth_year, respondent_address, respondent_occupation, respondent_marital_status, respondent_inviter, JSON.stringify(data));
      const saved = db.prepare('SELECT * FROM responses WHERE id = ?').get(id);
      recordParticipant(db, { ...saved, project_name: project.name, survey_title: survey.title });
    })();
