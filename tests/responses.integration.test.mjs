...
test('submission, listing and CSV retain all profile fields and answers', async () => {
  ...
  try {
    db.exec(`CREATE TABLE projects (id TEXT, name TEXT, status TEXT, start_date TEXT, end_date TEXT, max_responses INTEGER);
    ...
    const saved = db.prepare('SELECT * FROM responses').get();
    ...
    const people = listParticipants(db).participants;
