...
export default function ImportParticipants({onClose,onSaved}){
 ...
 return <Modal isOpen onClose={()=>{if(!busy)onClose();}} title="Nhập data từ Excel" size="lg">
  <p className="form-hint">Trang đầu tiên có hàng tiêu đề: Tên, Năm sinh, Số điện thoại, Địa chỉ, Nghề nghiệp, Tình trạng hôn nhân, Dự án tham gia, Người mời. Tên, Số điện thoại, Nghề nghiệp và Người mời là các mục bắt buộc; các mục còn lại có thể để trống hoặc không có cột. Người mời: Khánh hoặc Tế.</p>
  <p className="form-hint">Năm sinh: từ 1900 đến năm hiện tại. Tình trạng hôn nhân: Độc thân, Đã kết hôn - chưa con, Đã kết hôn - có con.</p>
  ...
  {result&&<><p role="status">{done?`Đã thêm ${result.added} hồ sơ`:`${result.valid} hồ sơ có thể nhập`} · {result.skipped} dòng trùng · {result.errors} dòng lỗi</p><div style={{overflow:'auto',maxHeight:350,margin:'16px 0'}}><table className="data-table"><thead><tr><th>Dòng</th><th>Tên</th><th>Năm sinh</th><th>Số điện thoại</th><th>Địa chỉ</th><th>Nghề nghiệp</th><th>Tình trạng hôn nhân</th><th>Dự án tham gia</th><th>Người mời</th><th>Kết quả</th></tr></thead><tbody>{result.rows.map(r=><tr key={r.row}><td>{r.row}</td><td>{r.name||'—'}</td><td>{r.birthYear||'—'}</td><td>{r.phone||'—'}</td><td>{r.address||'—'}</td><td>{r.occupation||'—'}</td><td>{r.maritalStatus||'—'}</td><td>{r.project||'—'}</td><td>{r.inviter||'—'}</td><td>{r.error||r.message||(done?'Đã thêm':'Sẵn sàng')}</td></tr>)}</tbody></table></div>{!done&&result.errors>0&&<p className="form-hint">Chỉ nhập các dòng hợp lệ. Sửa các dòng lỗi trong file rồi nhập lại; hồ sơ đã nhập sẽ được bỏ qua.</p>}</>}
