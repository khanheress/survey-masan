...
export async function GET() {
  ...
  try {
    const db = getDb();
    const forms = session.user.role === 'admin'
      ? db.prepare('SELECT id FROM recall_forms ORDER BY created_at DESC, id').all()
      : db.prepare('SELECT id FROM recall_forms WHERE created_by = ? ORDER BY created_at DESC, id').all(session.user.id);
    return NextResponse.json(forms.map(form => getRecallForm(db, form.id)));
  } catch {
...
export async function POST(request) {
  ...
  if (!session) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  try {
    return NextResponse.json(saveRecallForm(getDb(), await request.json(), session.user), { status: 201 });
  } catch (error) {
