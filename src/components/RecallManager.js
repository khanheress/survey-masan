...
export default function RecallManager({projectId,projectName}) {
  const { addToast } = useToast();
  const [editor, setEditor] = useState(null);
  const [bookingForm, setBookingForm] = useState(null);
  const load = useCallback(async signal => {
    const response = await fetch(`/api/recall?project_id=${encodeURIComponent(projectId)}`, { signal });
  ...
  return <div className="animate-fadeIn">
    <div className="page-header"><div><h2 className="page-title">Form Recall · {projectName}</h2><p className="data-description">Link đăng ký và lịch hẹn riêng cho dự án.</p></div>{projectId!=='unassigned'&&<button className="btn btn-primary" onClick={() => setEditor({})}>+ Tạo Form Recall</button>}</div>
    ...
    <Modal isOpen={!!editor} onClose={() => setEditor(null)} title={editor?.id ? 'Chỉnh sửa Form Recall' : 'Tạo Form Recall'} size="lg">
      {editor && <RecallEditor projectId={projectId} projectName={projectName} key={editor.id || 'new'} initialForm={editor.id ? editor : null} onClose={() => setEditor(null)} onSaved={() => { setEditor(null); refresh(); addToast('Đã lưu Form Recall.', 'success'); }} />}
    </Modal>
