...
export async function GET(request){
 try{
  ...
  const active_projects=await Promise.all(running.map(async p=>{const rules=parseProjectRules(p.rules_json);return {...p,rules,quota_counts:await projectQuotaCounts(db,p.id,rules),availability:getSurveyAvailability(p,p.response_count)};}));
